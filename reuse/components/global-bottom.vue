...
const formatter = computed(() => (currentSlideRoute.value.meta?.slide as { frontmatter: { glow: string, glowOpacity: number, glowHue: number, glowSeed: string | boolean } })?.frontmatter || {});
...
const seed = computed<string>(() => (formatter.value.glowSeed === 'false' || formatter.value.glowSeed === false)
	? Date.now().toString()
	: String(formatter.value.glowSeed) || 'default',
);
const overflow = 0.3;
const disturb = 0.3;
...
function usePloy(number = 16) {
	...
	function jumpPoints() {
		...
		points.value = points.value.map((o) => {
			...
			newPoints.delete(closest as Range);
			return closest;
		}) as Range[];
	}
...
<template>
	<div>
		...
		>
			<div
				class="clip bg-gradient-to-r from-[#22c55e] to-[#15803d]"
				:style="{ 'clip-path': `polygon(${poly1})`, 'opacity': opacity }"
			/>
			<div
				class="clip bg-gradient-to-l from-[#38bdf8] to-[#0369a1]"
				:style="{ 'clip-path': `polygon(${poly2})`, 'opacity': opacity }"
			/>
			<div
				class="clip bg-gradient-to-t from-[#86efac] to-[#67e8f9]"
